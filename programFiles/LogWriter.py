...
def __getLogFileName(logOutFolder, problemStateSize, maximumDepth, pTimeout, dTimeout, gAllocation, problem, plan, timeoutBool, valid):
    """generates the path for the current log file
    includes the following info in the path:
        state size
        maximum depth
        problem timeout
        depth timeout
        geometric allocation common factor
        plan validity
        if invalid reason plan invalid
        """
...
def writeLogs(problem, logOutFolder, results, valid, problemStateSize, realStateSize, pTimeout, dTimeout, maximumDepth, gAllocation, overwrite = False):
    ...
    fileName = __getLogFileName(logOutFolder, problemStateSize, maximumDepth, pTimeout, dTimeout, gAllocation, problem, plan, timeoutBool, valid)
    if overwrite:
...
def corrolate(root):
    """builds more complex and useful log forms:
        averaging together results of different runs
        combining different state sizes of same problem
        making tables of state size v depth for different problems in csv and md format"""
    files = list()
    for p, s, f in os.walk(root):
        for file in f:
            toAdd = os.path.join(p, file)
            if "logs/constructed" not in toAdd:
                files.append(toAdd)
    ...
    for n in problemFiles:
        ...
        currentFileList = sortedNicely(currentFileList)
        combinedString = ""
        depths = dict()
        states = list()
        ...
        usedStates = sortedNicely(usedStates)
        usedStates.reverse()
...
if __name__ == "__main__":
    """automatically create advanced logs for specified folder"""
    try:
